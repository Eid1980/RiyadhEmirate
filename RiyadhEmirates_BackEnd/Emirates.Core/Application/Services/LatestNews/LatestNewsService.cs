...
namespace Emirates.Core.Application.Services.LatestNews
{
    public class LatestNewsService : BaseService, ILatestNewsService
    {
        ...
        {
            var latestNews = _emiratesUnitOfWork.LatestNews.FirstOrDefault(l => l.Id.Equals(id));
            if (latestNews == null)
        ...
        {
            var serchResult = _emiratesUnitOfWork.LatestNews.GetQueryable().Where(x => x.IsActive).Include(x => x.NewsCateguery)
        ...
        public IApiResponse Delete(int id)
        {
            ...
            _emiratesUnitOfWork.News.Remove(latestNews);
            _emiratesUnitOfWork.Complete();
