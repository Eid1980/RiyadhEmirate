...
{
    public class RequestLandsInfringementService : BaseService, IRequestLandsInfringementService
    {
        private readonly IEmiratesUnitOfWork _emiratesUnitOfWork;
        private readonly IMapper _mapper;
        public RequestLandsInfringementService(IEmiratesUnitOfWork emiratesUnitOfWork, IMapper mapper)
        {
        ...
        {
            var request = _emiratesUnitOfWork.Requests.FirstOrDefault(l => l.Id.Equals(id), x => x.RequestLandsInfringement, x => x.Stage);
            if (request == null)
                throw new NotFoundException(typeof(Request).Name);
            var response = _mapper.Map<RequestLandsInfringementDto>(request.RequestLandsInfringement);
            if (response == null)
        ...
        public IApiResponse GetDetailsById(Guid id)
        {
            var request = _emiratesUnitOfWork.Requests.FirstOrDefault(l => l.Id.Equals(id),
                x => x.RequestLandsInfringement,
                x => x.RequestLandsInfringement.RequestType,
                x => x.RequestLandsInfringement.Governorate,
                x => x.Stage, x => x.Service);
            if (request == null)
                throw new NotFoundException(typeof(Request).Name);
            var response = _mapper.Map<GetRequestLandsInfringementDetailsDto>(request);
            if (response == null)
                throw new BusinessException("بيانات الطلب غير صحيحة, برجاء اختيار الطلب بطريقة صحيحة");
            return GetResponse(data: response);
        }
        public IApiResponse Create(CreateRequestLandsInfringementDto createModel)
        {
            if (CanCreate(createModel.UserId))
            {
                ...
                {
                    ServiceId = (int)SystemEnums.Services.LandsInfringement,
                ...
                var addedModel = _mapper.Map<RequestLandsInfringement>(createModel);
                addedModel.Id = request.Id;
                _emiratesUnitOfWork.RequestLandsInfringements.Add(addedModel);
        ...
        {
            var request = _emiratesUnitOfWork.Requests.FirstOrDefault(x => x.Id.Equals(updateModel.Id), x => x.Stage, x => x.RequestLandsInfringement);
            if (request == null)
                throw new NotFoundException(typeof(Request).Name);
            ...
            _emiratesUnitOfWork.RequestLandsInfringements.Update(request.RequestLandsInfringement, _mapper.Map<RequestLandsInfringement>(updateModel));
            _emiratesUnitOfWork.Complete();
            return GetResponse(message: CustumMessages.UpdateSuccess(), data: updateModel.Id);
        }
        private bool CanCreate(int userId)
        {
            return !_emiratesUnitOfWork.Requests.Where(x => x.ServiceId.Equals((int)SystemEnums.Services.LandsInfringement) && x.CreatedBy.Equals(userId) &&
