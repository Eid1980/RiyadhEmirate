...
{
    public class RequestMarriageCertificateService : BaseService, IRequestMarriageCertificateService
    {
        private readonly IEmiratesUnitOfWork _emiratesUnitOfWork;
        private readonly IMapper _mapper;
        public RequestMarriageCertificateService(IEmiratesUnitOfWork emiratesUnitOfWork, IMapper mapper)
        {
        ...
        {
            var request = _emiratesUnitOfWork.Requests.FirstOrDefault(l => l.Id.Equals(id), x => x.RequestMarriageCertificate, x => x.Stage);
            if (request == null)
                throw new NotFoundException(typeof(Request).Name);
            var response = _mapper.Map<RequestMarriageCertificateDto>(request.RequestMarriageCertificate);
            if (response == null)
        ...
        public IApiResponse GetDetailsById(Guid id)
        {
            var request = _emiratesUnitOfWork.Requests.FirstOrDefault(l => l.Id.Equals(id),
                x => x.RequestMarriageCertificate,
                x => x.RequestMarriageCertificate.RequestType,
                x => x.RequestMarriageCertificate.MaritalStatus,
                x => x.Stage, x => x.Service);
            if (request == null)
                throw new NotFoundException(typeof(Request).Name);
            var response = _mapper.Map<GetRequestMarriageCertificateDetailsDto>(request);
            if (response == null)
                throw new BusinessException("بيانات الطلب غير صحيحة, برجاء اختيار الطلب بطريقة صحيحة");
            return GetResponse(data: response);
        }
        public IApiResponse Create(CreateRequestMarriageCertificateDto createModel)
        {
            if (CanCreate(createModel.UserId))
            {
                ...
                {
                    ServiceId = (int)SystemEnums.Services.MarriageCertificate,
                ...
                var addedModel = _mapper.Map<RequestMarriageCertificate>(createModel);
                addedModel.Id = request.Id;
                _emiratesUnitOfWork.RequestMarriageCertificates.Add(addedModel);
        ...
        {
            var request = _emiratesUnitOfWork.Requests.FirstOrDefault(x => x.Id.Equals(updateModel.Id), x => x.Stage, x => x.RequestMarriageCertificate);
            if (request == null)
                throw new NotFoundException(typeof(Request).Name);
            ...
            _emiratesUnitOfWork.RequestMarriageCertificates.Update(request.RequestMarriageCertificate, _mapper.Map<RequestMarriageCertificate>(updateModel));
            _emiratesUnitOfWork.Complete();
            return GetResponse(message: CustumMessages.UpdateSuccess(), data: updateModel.Id);
        }
        private bool CanCreate(int userId)
        {
            return !_emiratesUnitOfWork.Requests.Where(x => x.ServiceId.Equals((int)SystemEnums.Services.MarriageCertificate) && x.CreatedBy.Equals(userId) &&
