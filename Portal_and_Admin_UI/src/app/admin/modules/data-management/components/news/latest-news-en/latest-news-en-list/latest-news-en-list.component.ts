...
@Component({
  selector: 'app-latest-news-en-list',
  templateUrl: './latest-news-en-list.component.html'
})
export class LatestNewsEnListComponent implements OnInit {
  ...
  ngOnInit() {
    this.globalService.setAdminTitle('أخر الأخبار انجليزي');
    this.pageSetting();
  }
  ...
  pageSetting() {
    ...
    this.pageListSettings = {
      PageTitle: 'قائمة أخر الأخبار انجليزي',
      listPermissionCode: '*',
      createButtonLink: '/admin/data-management/latest-news-en-add',
      createButtonText: 'انشاء خبر انجليزي جديد',
      ...
      actions: [
        {
          title: 'تعديل',
          routerLink: '/admin/data-management/latest-news-en-edit',
          IsQueryParams: true,
        ...
        {
          title: 'التفاصيل',
          routerLink: '/admin/data-management/latest-news-en-view',
          IsQueryParams: true,
  ...
  changecommentStatus(id: number, e: any) {
    this.newsService.changecommentStatus(id).subscribe((result) => {
      if (result.isSuccess) {
        this.list.getData();
      }
    });
