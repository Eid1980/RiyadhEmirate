...
@Component({
  selector: 'app-governorate-news-ar-edit',
  templateUrl: './governorate-news-ar-edit.component.html'
})
export class GovernorateNewsArEditComponent implements OnInit {
  ...
  getDetails() {
    this.newsService.getById(this.id).subscribe((response) => {
      ...
      if (!this.updateNewsDto.isArabic || this.updateNewsDto.newsTypeId != NewsTypes.GovernorateNews) {
        this.globalService.navigate("/admin/data-management/governorate-news-ar-list");
      }
      //#region Set Date
  ...
  onSubmit() {
    ...
    if (this.updateNewsform.valid) {
      ...
      this.updateNewsDto.newsTypeId = NewsTypes.GovernorateNews;
