...
export class EmiratesPrinceAddComponent implements OnInit {
  ...
  buildForm() {
    this.createEmiratesPrinceForm = this.formBuilder.group({
      ...
      behalfToArAr: [this.createEmiratesPrinceDto.behalfToArAr || null],
      behalfToArEn: [this.createEmiratesPrinceDto.behalfToArEn || null],
    ...
    if (this.createEmiratesPrinceForm.valid) {
      ...
      this.emiratesPrinceService.create(this.createEmiratesPrinceDto).subscribe((response) => {
          ...
          if (response.isSuccess) {
            if (this.createEmiratesPrinceForm.get('image').value) {
              let id = response.data.toString();
              this.fileManagerService.upload(id, 'EmiratesPrince', '', [this.createEmiratesPrinceForm.get('image').value]).subscribe(res => {
