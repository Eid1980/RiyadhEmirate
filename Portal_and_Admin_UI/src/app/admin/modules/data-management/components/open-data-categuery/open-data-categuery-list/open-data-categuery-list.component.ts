...
@Component({
  selector: 'app-open-data-categuery-list',
  templateUrl: './open-data-categuery-list.component.html'
})
export class OpenDataCategueryListComponent implements OnInit {
  @ViewChild(PageListComponent, { static: true }) list: PageListComponent;
  pageListSettings: PageListSetting;

  constructor(private openDataCategueryService: OpenDataCategueryService, private globalService: GlobalService) { }

  ngOnInit() {
    this.globalService.setAdminTitle('مجموعات البيانات المفتوحة');
    this.pageSetting();
  }

  pageSetting() {
    this.pageListSettings = {
      PageTitle: 'البحث في مجموعات البيانات المفتوحة',
      listPermissionCode: '*',
      createButtonLink: '/admin/data-management/open-data-categuery-add',
      createButtonText: 'اضافة مجموعة جديدة',
      Url: this.openDataCategueryService.serviceUrl,

      cols: [
        { Field: 'id', Header: 'الكود', Searchable: false, Hidden: true },
        { Field: 'nameAr', Header: 'الاسم عربي' },
        { Field: 'nameEn', Header: 'الاسم انجليزي' },
        { Field: 'openDataSubCategueryName', Header: 'التصنيف' },
        {
          Field: 'isActive',
          Header: 'الحالة',
          Searchable: false,
      ...
      actions: [
        {
          title: 'تعديل',
          routerLink: '/admin/data-management/open-data-categuery-edit',
          IsQueryParams: true,
        ...
        {
          title: 'التفاصيل',
          routerLink: '/admin/data-management/open-data-categuery-view',
          IsQueryParams: true,
  ...
  changeStatus(id: number, e: any) {
    this.openDataCategueryService.changeStatus(id).subscribe((result) => {
      if (result.isSuccess) {
        this.list.getData();
      }
    });
  ...
  isconfirm(id: number) {
    this.openDataCategueryService.delete(id).subscribe((result) => {
      if (result.isSuccess) {
