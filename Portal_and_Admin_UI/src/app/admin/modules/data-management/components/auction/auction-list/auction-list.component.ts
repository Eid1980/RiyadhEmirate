...
import { AuctionService } from '@proxy/auctions/auction.service';
...
export class AuctionListComponent implements OnInit {
  ...
  pageSetting() {
    this.pageListSettings = {
      PageTitle: 'البحث في الاعلانات والمزادات والمناقصات',
      listPermissionCode: '*',
      createButtonLink: '/admin/data-management/auction-add',
      createButtonText: 'اضافة مناقصة جديدة',
      Url: this.auctionService.serviceUrl,

      cols: [
        { Field: 'id', Header: 'الكود', Searchable: false, Hidden: true },
        { Field: 'title', Header: 'العنوان' },
        { Field: 'content', Header: 'المحتوى' },
        {
          Field: 'isActive',
          Header: 'الحالة',
          Searchable: false,
      ...
      actions: [
        {
          title: 'تعديل',
          routerLink: '/admin/data-management/auction-edit',
          IsQueryParams: true,
        ...
        {
          title: 'التفاصيل',
          routerLink: '/admin/data-management/auction-view',
          IsQueryParams: true,
  ...
  changeStatus(id: number, e: any) {
    this.auctionService.changeStatus(id).subscribe((result) => {
      if (result.isSuccess) {
        this.list.getData();
      }
    });
  ...
  isconfirm(id: number) {
    this.auctionService.delete(id).subscribe((result) => {
      if (result.isSuccess) {
