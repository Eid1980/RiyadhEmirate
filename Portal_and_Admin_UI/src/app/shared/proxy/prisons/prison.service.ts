...
import { CreatePrisonDto, GetPrisonDetailsDto, UpdatePrisonDto } from './models';
...
export class PrisonService {
  serviceUrl: string = `${environment.ApiUrl}/api/Prison`;
  constructor(public httpClient: HttpClient) {
  }

  getById = (id: number): Observable<ApiResponse<GetPrisonDetailsDto>> => {
    return this.httpClient.get<ApiResponse<GetPrisonDetailsDto>>(`${this.serviceUrl}/GetById/${id}`).pipe(
    );
  }

  create = (createdDto: CreatePrisonDto): Observable<ApiResponse<number>> => {
    return this.httpClient.post<ApiResponse<number>>(`${this.serviceUrl}/Create`, createdDto).pipe();
  }
  update = (updatedDto: UpdatePrisonDto): Observable<ApiResponse<number>> => {
